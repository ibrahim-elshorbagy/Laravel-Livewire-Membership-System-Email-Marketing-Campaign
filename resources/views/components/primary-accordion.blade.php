...
<div x-data="{ isExpanded: {{ json_encode($isExpandedByDefault) }} }">
    <button type="button"
        class="flex gap-4 justify-between items-center p-4 w-full text-left bg-neutral-100 underline-offset-2 hover:bg-neutral-100/75 focus-visible:bg-neutral-100/75 focus-visible:underline focus-visible:outline-hidden dark:bg-neutral-700 dark:hover:bg-neutral-700/75 dark:focus-visible:bg-neutral-700/75"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke="currentColor"
            class="transition size-5 shrink-0" aria-hidden="true" x-bind:class="isExpanded ? 'rotate-180' : ''">
    ...
    <div x-show="isExpanded" x-transition:enter="transition-all ease-out duration-300"
        ...
        x-transition:leave-end="opacity-0 max-h-0" class="overflow-hidden p-4 text-sm sm:text-base text-pretty">
        {{ $slot }}
    </div>
</div>