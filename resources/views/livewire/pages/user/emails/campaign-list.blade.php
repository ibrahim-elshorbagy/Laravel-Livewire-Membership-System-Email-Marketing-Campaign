<div
    ...
    <header class="flex flex-col items-center justify-between mb-6 md:flex-row">
        ...
        <div class="mt-4 md:mt-0">
            ...
            <a href="{{ route('user.emails.campaign.form') }}"
        ...
        <div class="flex flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4 md:items-center">
            ...
            <div class="flex flex-wrap gap-2">
                <x-primary-select-input wire:model.live="sortField" class="w-full sm:w-40">
                    <option value="campaign_title">Sort by Title</option>
        ...
        <table class="w-full text-sm text-left text-neutral-600 dark:text-neutral-400">
            ...
            <tbody class="divide-y divide-neutral-300 dark:divide-neutral-700">
                ...
                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800">
                    <td class="p-4">
                        <input type="checkbox" wire:model.live="selectedTemplates" value="{{ $campaign->id }}"
                            class="rounded">
                    </td>
                    <td class="p-4">{{ $campaign->campaign_title }}</td>
                    <td class="p-4">{{ $campaign->email_subject }}</td>
                    <td class="p-4">
                        <span
                            class="inline-flex px-2 py-1 text-xs rounded-full
                            {{ $campaign->sending_status === 'RUN' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                            {{ $campaign->sending_status }}
                        </span>
                    </td>
                    <td class="p-4">{{ $campaign->created_at->format('M d, Y H:i') }}</td>
                    <td class="p-4">
                        <div class="flex space-x-2">
                            <button wire:click="setActiveTemplate({{ $campaign->id }})"
                                class="inline-flex items-center px-2 py-1 text-xs {{ $campaign->sending_status === 'RUN' ? 'text-yellow-500 bg-yellow-500/10 hover:bg-yellow-500/20' : 'text-green-500 bg-green-500/10 hover:bg-green-500/20' }} rounded-md">
                                {{ $campaign->sending_status === 'RUN' ? 'Pause' : 'Run' }}
                            </button>

                            <a href="{{ route('user.emails.campaign.form', $campaign->id) }}"
                                class="inline-flex items-center px-2 py-1 text-xs text-blue-500 rounded-md bg-blue-500/10 hover:bg-blue-500/20">
                                Edit
                            </a>

                            <button wire:click="deleteTemplate({{ $campaign->id }})"
